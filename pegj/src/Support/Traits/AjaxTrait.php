...
trait AjaxTrait
{
        ...
        if (!empty($ajaxActions)) {
            ...
            foreach ($ajaxActions as $ajaxAction) {
                ...
                if ($context === 'admin') { // admin related ajax always needs to be private
                    add_action('wp_ajax_'. pegj_get_plugin_domain() .'_'. $actionName, [$this, $ajaxAction]);
                } else {
                    if (str_contains($ajaxAction, 'handle_ajax_nopriv_')) {
                        add_action( 'wp_ajax_nopriv_'. pegj_get_plugin_domain() .'_'. $actionName, [$this, $ajaxAction] );
                    }
    ...
    {
        return check_ajax_referer( pegj_get_plugin_domain() .'_'. $nonceName, 'nonce' );
    }
    ...
    {
        return wp_create_nonce( pegj_get_plugin_domain() .'_'. $nonceName );
    }
}